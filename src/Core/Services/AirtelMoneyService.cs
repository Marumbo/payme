    ...
    public class AirtelMoneyService : IPaymentService
    {
        ...
        public bool IsDeposit()
        {
                      /* sample messages for the regex below
                        ...
                        var message3 = "Dear Customer, money transfer to 999057130, Dali Mizaya is successful, Trans Id: PP200815.0822.G60250,Trans Amt: MK2000.00. Your available balance MK110.41";

                        */
                        
                        var findFirst = new Regex(@"(^( [tT]rans ID: |[tT]rans ID: |[Tt]rans.ID|Trans Id:|Dear Customer))");
                        var airtimeRegex = new Regex(@"[aA]irtime");
                        var findReceived = new Regex(@"(\b(money transfer)|(you have received)|(successful)\b)");
